...
# Read all zip files
inDir <- "~/ownCloud/PROFOUND_Data/Processed/ISIMIP2BLC"
...
if ( "CLIMATE_ISIMIP2BLBC_master" %in% dbListTables(db))   dbSendQuery(db, "DROP TABLE CLIMATE_ISIMIP2BLBC_master")

# create table in DB (cant type minus)
dbSendQuery(conn = db,
            "CREATE TABLE CLIMATE_ISIMIP2BLBC_master
...
dbListFields(db, "CLIMATE_ISIMIP2BLBC_master") # The fields in the table
## Close connnection to db
dbDisconnect(db)
# dbSendQuery(db, "DROP TABLE ISIMIP2BLBC")
