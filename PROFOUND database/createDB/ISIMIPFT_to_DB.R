...
# Read all zip files
inDir <- "~/ownCloud/PROFOUND_Data/Processed/ISIMIPFT"
...
if ( "CLIMATE_ISIMIPFT_master" %in% dbListTables(db))  dbSendQuery(db, "DROP TABLE CLIMATE_ISIMIPFT_master")

# create table in DB (cant type minus)
dbSendQuery(conn = db,
            "CREATE TABLE CLIMATE_ISIMIPFT_master
...
dbListFields(db, "CLIMATE_ISIMIPFT_master") # The fields in the table
## Close connnection to db
dbDisconnect(db)
# dbSendQuery(db, "DROP TABLE ISIMIPFT")
...
for (i in 1:length(filenames)){
  ...
  if (file.site == "Solling"){
    ...
    # open connection to DB
    db <- dbConnect(SQLite(), dbname= myDB)
    dbWriteTable(db, "CLIMATE_ISIMIPFT_master",
                 df[,columns],
...
# open connection to DB
db <- dbConnect(SQLite(), dbname=myDB)
# create index for variables we are going to query: so far location, forcingDataset, forcingConditions
# --> change names to include the table
dbGetQuery(db,"CREATE INDEX index_CLIMATE_ISIMIPFT_master_site_id ON CLIMATE_ISIMIPFT_master (site_id)")
dbGetQuery(db,"CREATE INDEX index_CLIMATE_ISIMIPFT_master_forcingDataset ON CLIMATE_ISIMIPFT_master (forcingDataset)")
dbGetQuery(db,"CREATE INDEX index_CLIMATE_ISIMIPFT_master_forcingConditions ON CLIMATE_ISIMIPFT_master (forcingConditions)")
dbDisconnect(db)
