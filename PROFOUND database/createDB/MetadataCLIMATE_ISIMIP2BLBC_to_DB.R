...
if ( "METADATA_CLIMATE_ISIMIP2BLBC_master" %in% dbListTables(db)){
  dbSendQuery(db, "DROP TABLE METADATA_CLIMATE_ISIMIP2BLBC_master")}
# create table in DB (cant type minus)
dbSendQuery(conn = db,
            "CREATE TABLE METADATA_CLIMATE_ISIMIP2BLBC_master
...
# Check the table
dbListFields(db, "METADATA_CLIMATE_ISIMIP2BLBC_master") # The fields in the table
...
# open connection to DB
db <- dbConnect(SQLite(), dbname= myDB)
dbWriteTable(db, "METADATA_CLIMATE_ISIMIP2BLBC_master", df[,columns], append=TRUE, row.names = FALSE)
dbDisconnect(db)
# reset the id values
...
dbGetQuery(db,"CREATE INDEX index_METADATA_CLIMATE_ISIMIP2BLBC_master_variable ON METADATA_CLIMATE_ISIMIP2BLBC_master (variable)")
dbGetQuery(db,"CREATE INDEX index_METADATA_CLIMATE_ISIMIP2BLBC_master_site_id ON METADATA_CLIMATE_ISIMIP2BLBC_master (site_id)")
## Close connnection to db
dbDisconnect(db)
