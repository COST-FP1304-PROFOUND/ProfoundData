...
dbGetQuery(db, "CREATE VIEW CLIMATE_ISIMIP2BLBC AS
           ...
           CLIMATE_ISIMIP2BLBC_master.forcingConditions,
...
for (i in 1:length(forcingDataset)){
  ids <- dbGetQuery(db, paste("SELECT site_id FROM (SELECT DISTINCT site_id FROM CLIMATE_ISIMIP2BLBC_master WHERE forcingDataset = '", 
                              forcingDataset[i], "')", sep = "")  )
  forcingConditions <- dbGetQuery(db, paste("SELECT forcingConditions FROM (SELECT DISTINCT forcingConditions FROM CLIMATE_ISIMIP2BLBC_master WHERE forcingDataset = '", 
                                            forcingDataset[i], "')", sep = "")  )[,1]
  for (k in 1:length(forcingConditions)){
    if ( paste("CLIMATE_ISIMIP2BLBC_", forcingDataset[i],"_",  forcingConditions[k], sep="") %in% dbListTables(db))  dbSendQuery(db, paste("DROP VIEW CLIMATE_ISIMIP2BLBC_",forcingDataset[i],"_",  forcingConditions[k], sep="") )
    
    dbGetQuery(db,  paste("CREATE VIEW CLIMATE_ISIMIP2BLBC_", forcingDataset[i],"_",  forcingConditions[k],  " AS ",
                          "SELECT CLIMATE_ISIMIP2BLBC_master.record_id, ",
                          "SITESID_master.site, ",
                          "CLIMATE_ISIMIP2BLBC_master.site_id, ",
                          "CLIMATE_ISIMIP2BLBC_master.date, ",
                          ...
                          "CLIMATE_ISIMIP2BLBC_master.wind_ms ",
                          "FROM CLIMATE_ISIMIP2BLBC_master INNER JOIN SITESID_master ON CLIMATE_ISIMIP2BLBC_master.site_id = SITESID_master.site_id  WHERE CLIMATE_ISIMIP2BLBC_master.forcingDataset = '",
                          forcingDataset[i], "' AND CLIMATE_ISIMIP2BLBC_master.forcingConditions = '", forcingConditions[k], "'", sep = "") )
    for (j in 1:length(ids[, 1])){
      if ( paste("CLIMATE_ISIMIP2BLBC_", forcingDataset[i],"_",  forcingConditions[k],"_",  ids[j,], sep="") %in% dbListTables(db))  dbSendQuery(db, paste("DROP VIEW CLIMATE_ISIMIP2BLBC_",forcingDataset[i],"_", forcingConditions[k],"_",  ids[j,] , sep="") )
      dbGetQuery(db,  paste("CREATE VIEW CLIMATE_ISIMIP2BLBC_", forcingDataset[i],"_",  forcingConditions[k], "_",  ids[j,],  " AS ",
                            "SELECT CLIMATE_ISIMIP2BLBC_master.record_id, ",
                            "SITESID_master.site, ",
                            "CLIMATE_ISIMIP2BLBC_master.site_id, ",
                            "CLIMATE_ISIMIP2BLBC_master.date, ",
                            ...
                            "CLIMATE_ISIMIP2BLBC_master.wind_ms ",
                            "FROM CLIMATE_ISIMIP2BLBC_master INNER JOIN SITESID_master ON CLIMATE_ISIMIP2BLBC_master.site_id = SITESID_master.site_id  WHERE CLIMATE_ISIMIP2BLBC_master.forcingDataset = '",
                            forcingDataset[i], "' AND CLIMATE_ISIMIP2BLBC_master.forcingConditions = '", forcingConditions[k], "' AND CLIMATE_ISIMIP2BLBC_master.site_id = '", ids[j,], "'", sep = "") )
