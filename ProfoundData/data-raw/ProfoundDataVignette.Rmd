...
author: "Ramiro Silveyra Gonzalez, Florian Hartig, Christopher Reyer"
date: "`r Sys.Date()`"
output: 
...
```{r dataTypes, include=FALSE, eval=T}
...
```{r sitesReport, include=FALSE, eval=T}
out <- NULL
...
data <- getData(location = "soro", dataset ="CLIMATE_ISIMIP2A", collapse = F)
names(data)
names(data[[1]])
```
...
```{r, echo=T,  message= F, warning=F}
data <- getData(location = "soro", dataset ="SOIL", collapse = F)
str(data, 1)
names(data[[1]])
```
...
```{r, echo=T,  message= F, warning=F}
data <- getData( location ="soro", dataset ="CLIMATE_ISIMIP2B", forcingDataset="GFDLESM2M", forcingCondition ="rcp2p6")
...
```{r, echo=T,  message= F, warning=F}
data <- getData( location ="soro", dataset ="CLIMATE_ISIMIP2B_GFDLESM2M_rcp2p6")
knitr::kable(head(data),  align = "l")
```
...
```{r, echo=T,  message= F, warning=F}
data <- getData( location ="hyytiala", dataset ="TREE", species = "Pinus sylvestris")
```
```{r, echo=T,  message= F, warning=F}
...
```{r, echo=T,  message= F, warning=F}
data <- getData("CLIMATE_LOCAL", "soro", period = c("2011-01-01","2012-12-31"), quality = 1, decreasing = F)

```

```{r, echo=T,  message= F, warning=F}
data <- getData("FLUX", "soro", period = c("2011-01-01","2012-12-31"), quality = 0, decreasing = T)
```
...
plotData("CLIMATE_LOCAL", "le_bray", automaticPanels = T)
```

Or for tree data
```{r, fig.show='hold',echo=T, results="hide", message= F, warning=F}
plotData("TREE", "solling_beech",   automaticPanels = T)
```
...
```{r, echo=T,  message= F, warning=F}
plotData(dataset ="CLIMATE_ISIMIP2B", location ="soro", forcingDataset="GFDLESM2M", forcingCondition ="rcp2p6", automaticPanels = T)
```
...
```{r, echo=T,  message= F, warning=F}
plotData( dataset ="TREE", location ="hyytiala", species = "Pinus sylvestris", automaticPanels = T)
```
...
```{r, echo=T,  message= F, warning=F}
plotData("CLIMATE_LOCAL", "soro", period = c("2011-01-01","2012-12-31"), quality = 1, decreasing = F)
```

```{r, echo=T,  message= F, warning=F}
plotData("FLUX", "soro", period = c("2011-01-01","2012-12-31"), quality = 0, decreasing = T)
```
- **aggregate**
...
plotData(dataset = "CLIMATE_ISIMIP2B",location ="soro",  forcingDataset= "GFDLESM2M",
           ...
           aggregate = "month", FUN =median, automaticPanels = F)
