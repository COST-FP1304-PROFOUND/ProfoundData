...
`r paste("The NDEPOSITION_EMEP data were extracted for for each forest site from the corresponding gridcell in the ISIMIP data described in ", df$reference, ". The dataset contains additionally the variable *forcingConditions*.", sep='') `
```{r, echo=F, warning = F, message=F}
df <- browseData("METADATA_NDEPOSITION_ISIMIP2B")
df <- df[df$variable == "forcingConditions", ]
rownames(df)<- NULL
``` 
...
The ENERGYBALANCE data contains half-hourly measurements of the variables listed in the table below and was obtained from  [FLUXNET2015](http://fluxnet.fluxdata.org/) data.
...
The energy balance data contains half-hourly measurements of the variables listed in the table below and was obtained from  [FLUXNET2015](http://fluxnet.fluxdata.org/) data.
